...
def calculate_estimate(params: dict) -> dict:
    ...
    # Septic Tank (Compulsory! No toggle control check)
    ...
    boq_items = [
        ...
        {"category": "Civil Works", "code": "CW-004", "description": f"Sand ({sand_type}) for RCC structural elements (Included in Package, Rate: Rs.{resolved_sand_rate}/Cft)", "unit": "Cft", "qty": sand_cft, "rate": 0, "amount": 0},
    ...
    if qty_40mm > 0:
        ...
        boq_items.append({"category": "Civil Works", "code": "CW-006a", "description": f"40 mm Coarse Aggregate (For PCC & Footings) (Included in Package, Rate: Rs.{rate_40mm}/Cft)", "unit": "Cft", "qty": qty_40mm, "rate": 0, "amount": 0})
    if qty_20mm > 0:
        rate_20mm = rates_by_code.get("AGGREGATE_20MM", 52.0)
        boq_items.append({"category": "Civil Works", "code": "CW-006b", "description": f"20 mm Coarse Aggregate (For RCC Works) (Included in Package, Rate: Rs.{rate_20mm}/Cft)", "unit": "Cft", "qty": qty_20mm, "rate": 0, "amount": 0})
    if qty_12mm > 0:
        rate_12mm = rates_by_code.get("AGGREGATE_12MM", 55.0)
        boq_items.append({"category": "Civil Works", "code": "CW-006c", "description": f"12 mm Coarse Aggregate (For Sunshades & Small RCC Works) (Included in Package, Rate: Rs.{rate_12mm}/Cft)", "unit": "Cft", "qty": qty_12mm, "rate": 0, "amount": 0})
    ...
    # Additional Works
    if compound_wall_cost > 0:
        boq_items.append({"category": "Additional Works", "code": "AD-001", "description": f"Compound Wall (Length: {compound_wall_length}ft, Height: {compound_wall_height}ft)", "unit": "Sqft", "qty": compound_wall_area, "rate": round(compound_wall_rate), "amount": round(compound_wall_cost)})
    if gate_cost > 0:
        boq_items.append({"category": "Additional Works", "code": "AD-002", "description": f"Entrance Gate (Size: {gate_width}x{gate_height}ft, {gate_package} Package)", "unit": "Sqft", "qty": gate_area, "rate": round(gate_rate), "amount": round(gate_cost)})
    if water_tank_cost > 0:
        boq_items.append({"category": "Additional Works", "code": "AD-003", "description": "Underground Sump (Water Tank)", "unit": "Litres", "qty": water_tank_capacity, "rate": round(water_tank_rate, 2), "amount": round(water_tank_cost)})
    if septic_tank_cost > 0:
        boq_items.append({"category": "Additional Works", "code": "AD-004", "description": "Septic Tank", "unit": "Litres", "qty": septic_tank_capacity, "rate": round(septic_tank_rate, 2), "amount": round(septic_tank_cost)})
    if elevation_cost > 0:
        boq_items.append({"category": "Additional Works", "code": "AD-005", "description": f"Front Designer Facade Elevation ({elevation_package} Package)", "unit": "Sqft", "qty": elevation_area, "rate": round(elevation_rate), "amount": round(elevation_cost)})
    if false_ceiling_cost > 0:
        boq_items.append({"category": "Additional Works", "code": "AD-006", "description": f"Designer False Ceiling ({false_ceiling_package} Package)", "unit": "Sqft", "qty": false_ceiling_area, "rate": round(false_ceiling_rate), "amount": round(false_ceiling_cost)})
    if wardrobe_cost > 0:
        boq_items.append({"category": "Additional Works", "code": "AD-007", "description": f"UPVC Wardrobes ({wardrobe_quality})", "unit": "Sqft", "qty": wardrobe_area, "rate": round(wardrobe_rate), "amount": round(wardrobe_cost)})
    if modular_kitchen_cost > 0:
        boq_items.append({"category": "Additional Works", "code": "AD-008", "description": f"Modular Kitchen Cabinets & Fittings ({modular_kitchen_package} Package)", "unit": "Sqft", "qty": modular_kitchen_area, "rate": round(modular_kitchen_rate), "amount": round(modular_kitchen_cost)})
    if interior_cost > 0:
        boq_items.append({"category": "Additional Works", "code": "AD-009", "description": f"Additional Interior Decoration Works ({interior_package} Package)", "unit": "Sqft", "qty": interior_area, "rate": round(interior_rate), "amount": round(interior_cost)})
